...
public class OnRenderMode : LightingMonoBehaviour {
    ...
    public void Initialize(LightMainBuffer2D mainBuffer) {
        ...
        LightmapPreset bufferPreset = mainBuffer.GetLightmapPreset();
    ...
    public void UpdateLoop() {
        ...
        if (mainBuffer.cameraBufferPreset.renderMode != CameraBufferPreset.RenderMode.Draw) {
			meshRenderer.enabled = false;
		}
    ...
    void UpdateLayer() {
        ...
        if (mainBuffer != null) {
            layer = mainBuffer.cameraSettings.GetLayerId(mainBuffer.cameraBufferPreset.id);
        }
