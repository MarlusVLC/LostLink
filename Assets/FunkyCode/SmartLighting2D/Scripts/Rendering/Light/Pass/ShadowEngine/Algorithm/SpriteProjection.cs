    ...
    public static class SpriteProjection {
        ...
        public static void Draw(List<Polygon2> polygons, float shadowDistance, float translucency) {
            ...
            for(int i = 0; i < PolygonCount; i++) {
                ...
                float length = Mathf.Lerp(ShadowEngine.shadowDistanceMin, ShadowEngine.shadowDistanceMax, step);
                ...
                for(float s = 0; s <= 1; s += 0.1f) {
                    ...
                    vA.x = edgeAWorldX;
                    vA.y = edgeAWorldY;
                    vA.x += Mathf.Cos(angleA) * shadowDistance * length;
                    vA.y += Mathf.Sin(angleA) * shadowDistance* length;

                    // Left Inner
                    vB.x = edgeBWorldX;
                    vB.y = edgeBWorldY;
                    vB.x += Mathf.Cos(angleB) * shadowDistance* length;
                    vB.y += Mathf.Sin(angleB) * shadowDistance* length;
