...
namespace CreativeSpore.SuperTilemapEditor
{
    ...
    public class MapTileNode : IPathNode
    {
        ...
        private bool RaycastCheck2D( Vector3 targetPosition )
        {
            ...
            int hitCount = Physics2D.RaycastNonAlloc(Position, dir, s_raycastHit2DCache, dir.magnitude, m_owner.RaycastDetectionLayerMask);
            bool raycastCheck = false;
            for (int i = 0; i < hitCount; i++)
            {
                if (s_raycastHit2DCache[i].distance == 0) // skip collisions from starting position (avoid collision with self)
                    continue;
                raycastCheck = true;
                break;
            }            
        ...
        private bool RaycastCheck3D(Vector3 targetPosition)
        {
            ...
            Vector3 dir = targetPosition - Position;
            int hitCount = Physics.RaycastNonAlloc(Position, dir, s_raycastHit3DCache, dir.magnitude, m_owner.RaycastDetectionLayerMask);
            bool raycastCheck = false;
            for (int i = 0; i < hitCount; i++)
            {
                if (s_raycastHit3DCache[i].distance == 0) // skip collisions from starting position (avoid collision with self)
                    continue;
                raycastCheck = true;
                break;
            }
    ...
    public class MapPathFinding
    {
        ...
        public int MaxIterations 
        {
            ...
            set { m_pathFinding.MaxIterations = value; }
        }
