...
namespace CreativeSpore.SuperTilemapEditor
{
    ...
    {
        static Shader s_spritesDiffuseShader = null;
        static Shader SpritesDiffuseShader { get { return s_spritesDiffuseShader ? s_spritesDiffuseShader : s_spritesDiffuseShader = Shader.Find("Sprites/Diffuse"); } }
        ...
        {
            m_tileColorList = Enumerable.Repeat(new TileColor32(clearColor), m_tileDataList.Count).ToList();
            m_needsRebuildMeshColor = true;
        }
        ...
        {
            return GetTileColor((int)(vLocalPos.x / CellSize.x), (int)(vLocalPos.y / CellSize.y));
        }

        public TileColor32 GetTileColor(int locGridX, int locGridY)
        {
            if (locGridX >= 0 && locGridX < m_width && locGridY >= 0 && locGridY < m_height)
            {
                int tileIdx = locGridY * m_width + locGridX;
                ...
                return m_tileColorList[tileIdx];
            }
            return TileColor32.invalid;
        }
...
#endif
                    if (!onlyPreview && m_meshRenderer.sharedMaterial.shader == SpritesDiffuseShader)
                    {
        ...
        private void UpdateSortOrder()
        {
            ...
            else if(ParentTilemap.SortOrder == STETilemap.eSortOrder.BottomRight)
            {
                // Flip the rows in chunks of 6
                for (int y = 0; y < m_height; ++y)
                {
                    for (int i = 0, j = m_width - 6; i < j; i += 6, j -= 6)
                    {
                        for (int k = 0; k < 6; ++k)
                        {
                            int temp = s_triangles[ y * m_width + i + k];
                            s_triangles[y * m_width + i + k] = s_triangles[j + k];
                            s_triangles[y * m_width + j + k] = temp;
                        }
                    }
                }
            }
            else if (ParentTilemap.SortOrder == STETilemap.eSortOrder.TopLeft)
            {
                // flip the columns in chunks of 6
                for (int i = 0, j = s_triangles.Count - m_width; i < j; i += m_width, j -= m_width)
                {
                ...
                for (int tx = 0; tx < m_width; ++tx, ++tileIdx)
                {
                    ...
                    if (tileData != Tileset.k_TileData_Empty)
                    {
                        ...
                        if (brushId > 0)
                        {
                            ...
                            if (tileBrush != null)
                            {
                                if (m_invalidateBrushes || (tileData & Tileset.k_TileFlag_Updated) == 0)
                                {
                                ...
                                if(tileBrush && 
                                    (tileData != (tileData = tileBrush.GetTileData(ParentTilemap, GridPosX + tx, GridPosY + ty, tileData)))
                                    )
                                {
                                    tileId = (int)(tileData & Tileset.k_TileDataMask_TileId);
                                    tile = Tileset.GetTile(tileId);
