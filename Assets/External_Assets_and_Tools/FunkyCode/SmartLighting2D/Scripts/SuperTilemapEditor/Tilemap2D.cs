                ...
                for (int x = tilemap.MinGridX; x <= tilemap.MaxGridX; x++) {
                    for (int y = tilemap.MinGridY; y <= tilemap.MaxGridY; y++) {
                        ...
                        if (shadowOptimization) {
                            bool left = tilemap.GetTile(x + 1, y) != null;
                            bool up = tilemap.GetTile(x, y + 1) != null;
                            bool right = tilemap.GetTile(x - 1, y) != null;
                            bool down = tilemap.GetTile(x, y - 1) != null;

                            tile.occluded = left && right && up && down;
                        }
